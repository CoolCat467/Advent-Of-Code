...
def to_grid(lines: list[str]) -> tuple[np.ndarray[tuple[int, int], np.dtype[np.int0]], int, int]:
...
def pawn_move(index: int,
              row_size: int,
              item_cap: int) -> list[int]:
    "Get positions pawn can move in"
...
def part_one(grid: np.ndarray[tuple[int, int], np.dtype[np.int0]],
             start: int, end: int) -> int:
    "Find length of shortest path from start to end"
    visited: set[int] = set()
...
def part_two(grid: np.ndarray[tuple[int, int], np.dtype[np.int0]],
             start: int, end: int) -> int:
    "Find length of shortest path that starts at lowest elevation to end"
    visited: set[int] = set()
    width, height = grid.shape
    item_cap = width * height

    unhandled: deque[int] = deque((idx for idx, v in enumerate(grid.flat) if v == 0))
