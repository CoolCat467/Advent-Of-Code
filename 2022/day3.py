...
"Advent of Code 2022 Day 3."  # noqa: D300
...
def prioritize(item_type: str) -> int:
    "Get priority of item type."  # noqa: D300
    code = ord(item_type)
    if code > 90:
        return code - 96  # - 97 + 1
...
def part_one(backpacks: list[Backpack]) -> int:
    "Get sum of the priorities of items in both slots of each backpack."  # noqa: D300
    total = 0
...
def part_two(backpacks: list[Backpack]) -> int:
    "Get the sum of the priorities of the group sticker for each group."  # noqa: D300
    total = 0
    cur_group: list[str] = []
    for slot_one, slot_two in backpacks:
