...
def part_one(stack: dict[int, deque[str]], instructions: list[tuple[int, int, int]]) -> str:
    "Get topmost crates from following instructions moving one box at a time"
...
def part_two(stack: dict[int, deque[str]], instructions: list[tuple[int, int, int]]) -> str:
    "Crane is actually pro and can move more than one box at once"
    for count, from_, to in instructions:
        values = []
        for _ in range(count):
...
def group_every(line: str, every: int, sep: int) -> list[str]:
    "Return groups from every {every} characters of line with separator"
    groups = []
    total_char = len(line)
    group_count = 0
...
##    file = open('day5.txt', encoding='utf-8')
    ...
    for line in file:
        ...
        for stack_idx, crate in enumerate(groups):
            if not stack_idx in stack:
                stack[stack_idx] = deque()
            if not crate:
                continue
            if crate.isdigit():
                break
            stack[stack_idx].append(crate[1:-1])
    file.close()
##    print(f'{stack = }')
