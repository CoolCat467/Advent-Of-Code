...
def points_in_line(line: tuple[tuple[int, int], ...]) -> set[tuple[int, int]]:
    "Calculate the points in a line given the edge points. Only right angles."
    last = Point(*line[0])
...
def print_map(walls: set[tuple[int, int]],
              sand: tuple[int, int],
              dead_sands: set[tuple[int, int]]) -> None:
...
def part_one(lines: list[tuple[tuple[int, int], ...]]) -> int:
    "Calculate many units of sand come to rest before rest falls to abyss"
...
def part_two(lines: list[tuple[tuple[int, int], ...]]) -> int:
    "Calculate many units of sand come to rest before sand source is blocked"
    walls: set[tuple[int, int]] = set()
    for line in lines:
        walls |= points_in_line(line)
...
##    file = open('day14.txt', encoding='utf-8')
    ...
    for line in file:
        lines.append(tuple(cast(tuple[int, int], tuple(map(int, v.split(','))))
