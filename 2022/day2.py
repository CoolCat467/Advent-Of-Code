...
def get_win(opponent: int, us: int) -> int:
    "Get if won against opponent given opponent action and our action"
    if opponent == us:
        return 3
    if (opponent+1)%3 == us:
        return 6
...
def score_strategy_incorrectly(strategy: list[tuple[str, str]]) -> int:
    "Score strategy if is list of opponent action and our action"
    score = 0
...
def get_action(opponent: int, win: int) -> int:
    "Get action go get game to win state given opponent action"
    if win == 3:
        return opponent
    if win == 0:
        us = opponent - 1
...
def score_strategy_correctly(strategy: list[tuple[str, str]]) -> int:
    "Score strategy if is list of opponent action and required game state"
    score = 0
