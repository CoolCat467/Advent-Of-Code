...
def cpu_generator() -> Generator[tuple[dict[str, int], tuple[Instruction | None, int], bool],
    ...
    while True:
        ...
        if instruction is not None:
            if instruction.name == 'noop':
                ...
                raise NotImplementedError(f'Instruction "{instruction.name}" not implemented')
...
def part_two(instructions: list[Instruction]) -> str:
    "Render sprite and get text answer"
    viewport = ""
    cycle = 0
    registers = {'X': 1}
    def wait(cycles: int) -> None:
