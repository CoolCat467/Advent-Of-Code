...
2) How many iterations untill syncronized?
"""
...
##        print(flash)
        ...
        while flashes:
            x, y = flashes.pop()
            surround = (value for value in self.circle_points(x, y) if not value in processed)
...
##        print(self.grid > 9)
    ...
    def check_sync(self):
        "Check if all octopus are syncronized."
    ...
    def sync_all(self):
        "Return number of steps required to sync all."
        steps = 0
...
def run():
    ...
    # Read file
    data = []
    with open('adv11.txt', 'r', encoding='utf-8') as rfile:
