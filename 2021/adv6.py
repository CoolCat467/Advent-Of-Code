...
class Fish:
    ...
    def tick(self):
        "Tick timer and return None if no new fish, or values to init new fish with."  # noqa: D300
        self.timer -= 1
        if self.timer >= 0:
            return None
...
class FishSim:
    "Fish Simulation."  # noqa: D300

    def __init__(self, fish):  # noqa: D107
        self.fish = fish

    def create_fish(self, *args):
        "Make new fish."  # noqa: D300
        fish = Fish(*args)
        self.fish.append(fish)

    def tick(self):
        "Process all fish by one day and make new."  # noqa: D300
        to_create = []
...
that will spawn in the future."""
    ...
    def fish_after(self, days):
        "Return the number of fish that will exist with unlimited resources after n days."  # noqa: D300
        for _ in range(days):
            future = {k - 1: v for k, v in self.future.items()}
            for day in range(self.delay + self.init_add + 1):
                if day not in future:
                    future[day] = 0
            if 0 not in future:
                continue
            now = future[0]
...
def run():
    ...
    # Read file
    data = []
    with open("adv6.txt", encoding="utf-8") as rfile:
