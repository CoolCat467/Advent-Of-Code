...
class Align:
    "Align points using fuel_move to score fuel consumption."
    def __init__(self, points, fuel_move):
        self.points = points
        self.fuel_move = fuel_move
    
    def min_align(self):
        "Return alignment with least fuel usage"
        alignments = {}
        for align in range(max(self.points)):
            fuel = 0
            for point in self.points:
                fuel += self.fuel_move(point, align)
            if not align in alignments:
                alignments[fuel] = []
...
def run():
    ...
    # read file
    data = []
    with open('adv7.txt', 'r', encoding='utf-8') as rfile:
