...
class Connection:
    "Base connection class."  # noqa: D300
    ...
    def read_bit(self):
        "Return 1 or 0, read from one bit."  # noqa: D300
        return int("".join(self.read(1)))

    def read_int(self, length):
        "Return length bits read as int from binary."  # noqa: D300
        if length == -1:
...
def run():
    ...
    # Read file
    data = []
    with open("adv16.txt", encoding="utf-8") as rfile:
