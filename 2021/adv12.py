...
def get_all_paths(point_map):
    ...
    def find_path(pos, current):
        ...
        for dest in point_map[pos]:
            if dest.isupper() or not dest in current:
...
def get_all_paths_small2(point_map):
    ...
    def find_path(pos, current):
        ...
        for dest in point_map[pos]:
            if not has_smol or dest.isupper() or not dest in current:
                find_path(dest, next_)
    find_path('start', [])
    return tuple(found)

def map_all_conn(point_map):
    "Make map have all possible starts and ends, not just hiding in end."
    data = point_map
    for end, values in tuple(point_map.items()):
        for start in values:
            if not start in data:
                data[start] = []
            if not end in data[start]:
                data[start].append(end)
            if not start in data[end]:
...
def run():
    ...
    # Read file
    data = []
    with open('adv12.txt', 'r', encoding='utf-8') as rfile:
