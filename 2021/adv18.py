...
def explode_number(snail_num):
    "Explode snail number."  # noqa: D300
    ...
    def fix_right(items, right=None, left=None):
        ...
        for idx, val in enumerate(items):
            ...
            values.append(val)
        if frm is None:
            if right is not None and isinstance(values[1], int):
                values[1] += right
                right = None
            if left is not None and isinstance(values[0], int):
                values[0] += left
                left = None
...
def run():
    ...
    # Read file
    data = []
    with open("adv18.txt", encoding="utf-8") as rfile:
