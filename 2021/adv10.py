...
def parse_line(line):
    ...
    while stack:
        ...
        add += CLOSES[value]
    error_score = sum(count*mult for count, mult in zip(errors.values(), ERROR_SCORES))
    return error_score, auto_score

def parse_lines(lines):
    "Parse lines and return error and autocomplete scores."
    error_score = 0
    auto_scores = []
    for line in lines:
...
def run():
    ...
    # Read file
    data = []
    with open('adv10.txt', 'r', encoding='utf-8') as rfile:
