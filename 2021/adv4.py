...
class BingoBoard:
    ...
    def drew(self, value):
        "Draw value, update data, and return bool of if won this round."
    ...
    def get_marked_unmarked(self):
        "Return dictionary, key is bool of if values were part of win."
        yesno = {0:[], 1:[]}
    ...
    def get_score(self):
        "Get the score of this board. 0 if not won."
        if not self.won_prev:
            return 0
...
class BoardControler:
    ...
    def read_file(cls, data):
        "Read data from file and return new class instance"
    ...
    def check_won(self):
        "Return list of boards that have won now or previously."
        won = []
    ...
    def total_won(self):
        "Return the number of boards that have won now or previously."
        return len(self.check_won())
    
    def waiting_count(self):
        "Return the number of boards we are waiting for to win."
        return len(self.boards) - self.total_won()
    
    def still_waiting(self):
        "Return True if still waiting for a board to win."
        return self.waiting_count() > 0
    
    def draw(self):
        "Draw a number, update boards, and return ones that win now."
    ...
    def get_scores(self):
        "Return score for each board as dictionary"
        scores = {}
        for idx, board in enumerate(self.boards):
...
def run():
    ...
    # Read file
    data = []
    with open('adv4.txt', 'r', encoding='utf-8') as rfile:
        data = rfile.read()
        rfile.close()
    # Solve 1
    controler = BoardControler.read_file(data)
    won = False
    has1 = False
    while controler.still_waiting():
        won = controler.draw()
        if won:
            scores = controler.get_scores()
            if not has1:
                print(scores[won[0]])
                has1 = True
    # Solve 2
